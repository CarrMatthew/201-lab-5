...
Write a function called multiply() that takes in two numbers as arguments and returns an array
 ...
 follows this example and concatenates a message using the arguments passed into the function:
...
follow this example and concatenates messages using the arguments that were passed into the function:
...
function sumAndMultiply(a,b,c){ //eslint-disable-line
  ...
  var numString = sum(a, sum(' and ', sum(b, sum(' and ', c)[0])[0])[0])[0];
  var message1 = sum(numString,  sum(' sum to ', sum(theSum,'.')[0])[0])[0];
  var message2 = sum('The product of ', sum(numString, sum(' is ', sum(theProduct, '.')[0])[0])[0])[0];
...
and then returns an array where the first element is the sum of the numbers in the array, and the second element is a 
...
function sumArray(testArray) {
  ...
  for(var i = 0; i < testArray.length; i = sum(i,1)[0]) {
    add = sum(add, testArray[i])[0];
    if(i === testArray.length - 1) {
      msg = sum(msg , testArray[i])[0];
    } else {
